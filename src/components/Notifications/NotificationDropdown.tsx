...
export default function NotificationDropdown({ userId }: { userId?: string | null }) {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      notificationsArray.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  const getNotificationText = (n: any) => {
    ...
    switch (n.type) {
      case "like": return `${mainText} liked your post ❤️`;
      case "comment": return `${mainText} commented: "${n.commentText}"`;
      case "follow": return `${mainText} started following you ➕`;
      case "tip": return n.message || `${mainText} sent you a tip 💰`;
      default: return `${mainText} interacted with you`;
    }
  };

  return (
    <Dropdown align="end" show={isOpen} onToggle={() => setIsOpen((prev) => !prev)}>
      ...
      <Dropdown.Menu style={{ width: "380px", maxHeight: "500px", overflowY: "auto" }}>
        <div className="d-flex justify-content-between align-items-center px-3 py-2 border-bottom sticky-top bg-white">
          ...
          {unreadCount > 0 && (
            <Button size="sm" variant="link" onClick={() => {/* markAllAsRead function call */}} style={{ fontSize: "0.8rem", textDecoration: 'none' }}>
              Mark all as read
            </Button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center text-muted py-3">No notifications yet</div>
        ) : (
          notifications.map((n) => {
            ...
            return (
              <Dropdown.Item
                key={n._id}
                className={`notification-item d-flex align-items-start gap-2 py-3 border-bottom ${n.read ? "opacity-75" : "bg-light fw-bold"}`}
              ...
              >
                <Image
                  src={senderData?.image || "https://res.cloudinary.com/dlcrwtyd3/image/upload/v1757470463/3135715_niwuv2.png"}
                  roundedCircle
                  style={{ width: "40px", height: "40px", flexShrink: 0, objectFit: "cover" }}
                />
