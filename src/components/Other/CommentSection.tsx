...
export default function CommentSection({ postId, currentUserId }: CommentSectionProps) {
  ...
  async function fetchComments(newPage = 1) {
    try {
      ...
      if (res.ok) {
        setComments(prev => newPage === 1 ? data.comments : [...prev, ...data.comments]);
  ...
  useEffect(() => { fetchComments(1); }, [postId]);

  // Helper functions for nested comments
  ...
  function replaceReplyInTree(comments: Comment[], tempId: string, savedReply: Comment): Comment[] {
    return comments.map(c => ({
      ...c,
      replies: c.replies ? c.replies.map(r => r._id === tempId ? savedReply : r) : [],
    }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setComments(prev => prev.map(c => c._id === optimisticComment._id ? newComment : c));
    } catch (error) {
  ...
  async function handleReplySubmit(parentId: string) {
    ...
    try {
      const res = await fetch("/api/comments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ postId, userId: currentUserId, text: optimisticReply.text, parentId }),
      });
      if (!res.ok) throw new Error("Failed to post reply");
      const savedReply = await res.json();
      setComments(prev => replaceReplyInTree(prev, optimisticReply._id, savedReply));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    channel.bind("new-comment", ({ comment }: { comment: Comment }) => {
      setComments(prev => comment.parentId ? addReplyToTree(prev, comment.parentId, comment) : [comment, ...prev]);
    });
    channel.bind("delete-comment", ({ commentId }: { commentId: string }) => {
      setComments(prev => removeCommentFromTree(prev, commentId));
    });
    return () => pusherClient.unsubscribe(`comments-${postId}`);
  }, [postId]);
        ...
        <div className="flex-grow-1">
          ...
          {replyingTo === comment._id && (
            <div className="mt-2">
              ...
              />
              <button className="btn btn-sm btn-primary" onClick={() => handleReplySubmit(comment._id)}>
                Reply
              </button>
            </div>
          )}
