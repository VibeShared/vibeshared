...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingLike) {
      ...
      await post.save();
      return NextResponse.json({ message: "Post unliked", likesCount: post.likesCount });
    }
    ...
    return NextResponse.json({ message: "Post liked", likesCount: post.likesCount });
  } catch (error) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(postId)) return NextResponse.json({ error: "Invalid Post ID format" }, { status: 400 });

    const likes = await Like.find({ postId }).populate("userId", "name image").lean();
